...
df['volatility_ratio'] = (
    df['WTI_Close'].pct_change().rolling(6).std() /
    (df['WTI_Close'].pct_change().rolling(24).std() + 1e-9)
)

# Heure encodée cycliquement (patterns horaires WTI)
df['hour_sin'] = np.sin(2 * np.pi * df.index.hour / 24)
df['hour_cos'] = np.cos(2 * np.pi * df.index.hour / 24)

# Volume relatif (spike de volume = signal)
df['volume_ratio'] = df['WTI_Volume'] / (df['WTI_Volume'].rolling(24).mean() + 1e-9)

# USD normalisé (variation au lieu de niveau absolu)
df['USD_return'] = df['USD_Open'].pct_change(12)
...
df['sentiment_available'] = df['sentiment_score'].notna().astype(int)  # flag avant ffill
...
FEATURES = [
    ...
    'session_europe', 'MACD_slope',
    'volatility_ratio', 'hour_sin', 'hour_cos',
    'volume_ratio', 'USD_return',
    'sentiment_score', 'sentiment_available',
]

df[FEATURES].dropna().to_csv('wti_features_v3.csv')
print("✅ wti_features_v4.csv exporté —", df[FEATURES].dropna().shape)